...
import { ResolveWikiCommentSchema } from '@/src/schemas/wiki-comment.schema'
...
export const PATCH = withAxiom(async (request: NextRequest, ctx: Params) => {
  ...
  const consent = await requireConsent(
    auth.value.user.id,
    'PATCH /api/workspaces/[id]/wiki/[wikiPageId]/comments/[commentId]/resolve',
  )
  ...
  const parsed = ResolveWikiCommentSchema.safeParse(body)
