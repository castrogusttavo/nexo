...
import { CreateWikiCommentSchema } from '@/src/schemas/wiki-comment.schema'
...
export const GET = withAxiom(async (_request: NextRequest, ctx: Params) => {
  ...
  const result = await WikiCommentService.list(
    auth.value.user.id,
    id,
    wikiPageId,
  )
...
export const POST = withAxiom(async (request: NextRequest, ctx: Params) => {
  ...
  const consent = await requireConsent(
    auth.value.user.id,
    'POST /api/workspaces/[id]/wiki/[wikiPageId]/comments',
  )
  ...
  const result = await WikiCommentService.create(
    auth.value.user.id,
    id,
    wikiPageId,
    parsed.data,
