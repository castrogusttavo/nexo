...
import { CreateCycleSchema } from '@/src/schemas/cycle.schema'
...
export const GET = withAxiom(async (_request: NextRequest, ctx: Params) => {
  ...
  const result = await CycleService.list(auth.value.user.id, id, slug)
...
export const POST = withAxiom(async (request: NextRequest, ctx: Params) => {
  ...
  const consent = await requireConsent(
    auth.value.user.id,
    'POST /api/workspaces/[id]/projects/[slug]/cycles',
  )
