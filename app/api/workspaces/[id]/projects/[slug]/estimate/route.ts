...
import { UpdateEstimateSettingsSchema } from '@/src/schemas/estimate.schema'
...
export const GET = withAxiom(async (_request: NextRequest, ctx: Params) => {
  ...
  const result = await EstimateService.get(auth.value.user.id, id, slug)
...
export const PATCH = withAxiom(async (request: NextRequest, ctx: Params) => {
  ...
  const consent = await requireConsent(
    auth.value.user.id,
    'PATCH /api/workspaces/[id]/projects/[slug]/estimate',
  )
  if (!consent.ok) return handleError(consent.error)

  const [{ id, slug }, body] = await Promise.all([ctx.params, request.json()])
  const parsed = UpdateEstimateSettingsSchema.safeParse(body)
