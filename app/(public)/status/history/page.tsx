...
function groupByMonth(incidents: IncidentSummaryDTO[]): Map<string, IncidentSummaryDTO[]> {
      ...
      <div className='w-full space-y-6'>
        ...
        {!result.ok || result.value.length === 0 ? (
          ...
          <div className='space-y-8'>
            {Array.from(groupByMonth(result.value).entries()).map(([month, items]) => (
              <div key={month} className='space-y-4'>
                <h4 className='capitalize'>{month}</h4>
                {items.map((inc) => {
                  const meta = STATUS_META[inc.severity]
                  const startedAt = new Date(inc.startedAt)
                  const dayNum = startedAt.toLocaleDateString('pt-BR', { day: '2-digit' })
                  const weekday = startedAt.toLocaleDateString('pt-BR', { weekday: 'short' })
                  return (
                    <div key={inc.id} className='w-full flex space-x-8 border-b border-zinc-700/80 pb-8'>
                      <Muted>
                        <strong className='text-primary'>{dayNum}</strong> {weekday}
                      </Muted>
                      <div className='w-full flex-1 flex gap-3'>
                        <div className={cn('h-full w-1', meta.bar)} />
                        <div className='w-full'>
                          <div className='w-full flex justify-between'>
                            <Link href={`/status/incident/${inc.id}`} className='font-medium hover:underline'>
                              {inc.title}
                            </Link>
