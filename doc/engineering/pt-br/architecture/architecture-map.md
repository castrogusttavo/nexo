...
erDiagram
    ...
    Workspace ||--o{ User : "possui membros"
    User ||--o{ Session : "possui sessões"
    User ||--o{ Account : "possui contas"
```
...
```mermaid
flowchart TD
    Start([Usuário]) --> Choice{Método de auth}

    Choice --> Email["Email + Senha"]
    ...
    BetterAuth --> SessionCreate["Criar Sessão\n(cookie cache 5min TTL)"]
    SessionCreate --> DB

    BetterAuth --> NewUser{Novo usuário?}
    NewUser -->|Sim| Welcome["Enviar E-mail de Boas-vindas\n(Resend)"]
    NewUser -->|Não| Done([Autenticado])
    Welcome --> Done

    BetterAuth --> Linking{Email já existe?}
    Linking -->|Sim| Link["Vincular Conta"]
    Link --> Done
    Linking -->|Não| Create["Criar Usuário + Conta"]
    Create --> NewUser
```
...
sequenceDiagram
    ...
    U ->> Browser: Acessar página protegida
