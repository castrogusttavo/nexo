...
export function FontSizeToolbarButton() {
  ...
  return (
    <div className='flex h-7 items-center gap-1 rounded-md bg-muted/60 p-0'>
      <ToolbarButton onClick={() => handleFontSizeChange(-1)} tooltip='Diminuir'>
        <Minus />
      </ToolbarButton>
      ...
      <Popover open={isFocused} modal={false}>
        ...
        />
        <PopoverContent className='w-10 px-px py-1' initialFocus={false}>
          {FONT_SIZES.map((size) => (
            <button
              key={size}
              className={cn(
                'flex h-8 w-full items-center justify-center text-sm hover:bg-accent data-highlighted:bg-accent'
              )}
              onClick={() => {
      ...
      <ToolbarButton onClick={() => handleFontSizeChange(1)} tooltip='Aumentar'>
        <Plus />
      </ToolbarButton>
    </div>
  )
