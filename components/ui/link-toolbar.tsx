...
export function LinkFloatingToolbar({ state }: { state?: LinkFloatingToolbarState }) {
  ...
  const input = (
    <div className='flex w-[330px] flex-col' {...inputProps}>
      <div className='flex items-center'>
        ...
        <FloatingLinkUrlInput className={inputVariants()} placeholder='Cole o link' data-plate-focus />
      </div>
      ...
      <div className='flex items-center'>
        ...
        <input className={inputVariants()} placeholder='Texto exibido' data-plate-focus {...textInputProps} />
      </div>
    </div>
  )
  ...
  const editContent = editState.isEditing ? (
    ...
    <div className='box-content flex items-center'>
      ...
      <button className={buttonVariants({ size: 'sm', variant: 'ghost' })} type='button' {...unlinkButtonProps}>
        <Unlink width={18} />
      </button>
    </div>
  )
...
function LinkOpenButton() {
  ...
  return (
    <a
      ...
      aria-label='Abrir link em uma nova aba'
      target='_blank'
    >
